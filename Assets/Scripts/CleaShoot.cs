...
public class CleaShoot : NetworkBehaviour
{
    ...
    private GameObject healZone; //zone de soin
    ...
    private void Start()
    {
        ...
        healZone = GameObject.Find("HealZone");
        playerController = GetComponent<PlayerController>();
        sphereCollider = GameObject.Find("HealZone").GetComponent<SphereCollider>(); //censé fonctionné
    }
    ...
    private void ShootMagic() //on lance les sorts magiques
    {
        ...
        Debug.Log("Lancement du sort de soin");
        StartCoroutine(magicAttack(healZone)); //on lance le sort
        
        
        //a faire pour tout les players qui isTrigger avec la sphereColider 
    ...
    private IEnumerator magicAttack(GameObject spell)
    {
        ...
        yield return new WaitForSeconds(3); //tps du soin
        spell.SetActive(false);
        useMagic = false;
        playerController.doSomethingFalse();
        yield return new WaitForSeconds(20); //tps de recharge
    }
    ...
    private void CmdPlayerHeal(string playerId)
    {
        Debug.Log(playerId + " a été touché");
        
        //gestion des dégats
        Player player = GameManager.getPlayer(playerId);
        player.RpcTakeHeal(10);
    }
